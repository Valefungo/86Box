...
{
    uintptr_t f_48k    = ((is_48k & 1) ? FM_FORCE_48K : 0) |
                         ((is_48k & 2) ? FM_CRYSTAL : 0);

    void *    flag_48k = (void *) f_48k;
    ...
    switch (chip_id) {
        ...
        case FM_YM3812: /* OPL2 */
            if (fm_driver == FM_DRV_NUKED) {
                *drv      = (is_48k & 1) ? nuked_opl2_drv_48k : nuked_opl2_drv;
                drv->priv = device_add_inst_params(&ym3812_nuked_opl2_device, fm_dev_inst[fm_driver][chip_id]++, flag_48k);
            } else {
                *drv      = ymfm_drv;
        ...
        case FM_YMF262: /* OPL3 */
            if (fm_driver == FM_DRV_NUKED) {
                *drv      = (is_48k & 1) ? nuked_opl3_drv_48k : nuked_opl3_drv;
        ...
        case FM_CQM: /* CQM */
            *drv      = (is_48k & 1) ? nuked_cqm_drv_48k : nuked_cqm_drv;
...
uint8_t
fm_driver_get_cs(int chip_id, fm_drv_t *drv)
{
    return fm_driver_get_ex(chip_id, drv, 2);
}
